...
@section('content')
  @component('dashboard.layouts.headers.auth')
    @component('dashboard.layouts.headers.breadcrumbs')
      <li class="breadcrumb-item"><a href="{{ route('lead.category.index') }}">{{ __('Category Management') }}</a></li>
          ...
          <div class="card-header">
            <div class="row align-items-center">
              ...
              <div class="col-4 text-right">
                <a href="{{ route('lead.category.create') }}" class="btn btn-sm btn-primary">{{ __('Add category') }}</a>
              </div>
            </div>
          </div>
                    ...
                    <td class="text-right">
                      <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                          <a class="dropdown-item" href="{{ route('lead.category.edit', $category) }}">{{ __('Edit') }}</a>
                          <form action="{{ route('lead.category.destroy', $category) }}" method="post">
                            @csrf
                            @method('delete')
