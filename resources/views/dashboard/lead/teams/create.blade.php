...
@section('pageTitle','Copa Tech - Add Team Member')

@section('content')
  @component('dashboard.layouts.headers.auth')
    @component('dashboard.layouts.headers.breadcrumbs')
      <li class="breadcrumb-item"><a href="{{ route('lead.team.index') }}">{{ __('Team Management') }}</a></li>
      <li class="breadcrumb-item active" aria-current="page">{{ __('Add Team Member') }}</li>
    @endcomponent
  @endcomponent
          ...
          <div class="card-header">
            <div class="row align-items-center">
              ...
              <div class="col-4 text-right">
                <a href="{{ route('lead.team.index') }}" class="btn btn-sm btn-primary">{{ __('Back to list') }}</a>
              </div>
            </div>
          </div>
          <div class="card-body">
            <form method="post" class="item-form" action="{{ route('lead.team.store') }}" autocomplete="off">
              @csrf
              ...
              <div class="pl-lg-4">
                <div class="form-group{{ $errors->has('user_id') ? ' has-danger' : '' }}">
                  ...
                  <select name="user_id" id="select-user" class="form-control{{ $errors->has('user_id') ? ' is-invalid' : '' }}" placeholder="{{ __('User') }}" autofocus>
                    <option value="">-</option>
                    @foreach ($users as $user)
                      <option value="{{ $user->id }}" {{ $user->id == old('user_id') ? 'selected' : '' }}>{{ $user->name }} - {{ $user->email }}</option>
                    @endforeach
                  </select>
                ...
                <div class="form-group{{ $errors->has('title') ? ' has-danger' : '' }}">
                  ...
                  <input type="text" name="title" id="input-title" class="form-control{{ $errors->has('title') ? ' is-invalid' : '' }}" placeholder="{{ __('Title') }}" value="{{ old('title') }}">
                ...
                <div class="form-group{{ $errors->has('twitter') ? ' has-danger' : '' }}">
                  ...
                  <input type="url" name="twitter" id="input-twitter" class="form-control{{ $errors->has('twitter') ? ' is-invalid' : '' }}" placeholder="{{ __('Twitter') }}" value="{{ old('twitter') }}">
                ...
                <div class="form-group{{ $errors->has('telegram') ? ' has-danger' : '' }}">
                  ...
                  <input type="url" name="telegram" id="input-telegram" class="form-control{{ $errors->has('telegram') ? ' is-invalid' : '' }}" placeholder="{{ __('Telegram') }}" value="{{ old('telegram') }}">
