...
@section('pageTitle','Copa Tech - Venue Management')

@section('content')
  @component('dashboard.layouts.headers.auth')
    @component('dashboard.layouts.headers.breadcrumbs')
      <li class="breadcrumb-item"><a href="{{ route('member.venue.index') }}">{{ __('Venue Management') }}</a></li>
            ...
            <div class="row align-items-center">
              <div class="col-8">
                ...
                <p class="text-sm mb-0">
                  {{ __('This is an example of venue management. This is a minimal setup in order to get started fast.') }}
                </p>
              </div>
              <div class="col-4 text-right">
                <a href="{{ route('member.venue.create') }}" class="btn btn-sm btn-primary">{{ __('Add venue') }}</a>
              </div>
            </div>
          </div>
            ...
            <table class="table align-items-center table-flush"  id="datatable-basic">
              ...
              <tbody>
                @foreach ($venues as $venue)
                  <tr>
                    <td><a href="{{ $venue->link }}" target="_blank">{{ $venue->name }}</a></td>
                    <td>{{ $venue->address }}</td>
                    <td class="text-right">
                      <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                          <a class="dropdown-item" href="{{ route('member.venue.edit', $venue) }}">{{ __('Edit') }}</a>
                          <form action="{{ route('member.venue.destroy', $venue) }}" method="post">
                            @csrf
                            @method('delete')
                            <button type="button" class="dropdown-item" onclick="confirm('{{ __("Are you sure you want to delete this venue?") }}') ? this.parentElement.submit() : ''">
                              {{ __('Delete') }}
                            </button>
                          </form>
                        </div>
