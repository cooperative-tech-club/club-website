...
@section('pageTitle','Copa Tech - Events Management')

@section('content')
  @component('dashboard.layouts.headers.auth')
    @component('dashboard.layouts.headers.breadcrumbs')
      <li class="breadcrumb-item"><a href="{{ route('communication.workshop.index') }}">{{ __('Events Management') }}</a></li>
            ...
            <div class="row align-items-center">
              <div class="col-8">
                ...
                <p class="text-sm mb-0">
                  {{ __('all year round events, conferences and trainings.') }}
                </p>
              </div>
              <div class="col-4 text-right">
                <a href="{{ route('communication.workshop.create') }}" class="btn btn-sm btn-primary">{{ __('Add Events') }}</a>
              </div>
            </div>
          </div>
                    ...
                    <td class="text-right">
                      <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                          <a class="dropdown-item" href="{{ route('communication.workshop.edit', $workshop) }}">{{ __('Edit') }}</a>
                          <form action="{{ route('communication.workshop.destroy', $workshop) }}" method="post">
                            @csrf
                            @method('delete')
