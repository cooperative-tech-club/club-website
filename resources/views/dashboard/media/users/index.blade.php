...
@section('content')
  @component('dashboard.layouts.headers.auth')
    @component('dashboard.layouts.headers.breadcrumbs')
      <li class="breadcrumb-item"><a href="{{ route('media.user.index') }}">{{ __('User Management') }}</a></li>
          ...
          <div class="card-header">
            <div class="row align-items-center">
              ...
              <div class="col-4 text-right">
                <a href="{{ route('media.user.create') }}" class="btn btn-sm btn-primary">{{ __('Add user') }}</a>
              </div>
            </div>
          </div>
                    ...
                    <td class="text-right">
                      <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                          @if ($user->id != auth()->id())
                            <a class="dropdown-item" href="{{ route('media.user.edit', $user) }}">{{ __('Edit') }}</a>
                            <form action="{{ route('media.user.destroy', $user) }}" method="post">
                              @csrf
                              @method('delete')
                          ...
                          @else
                            <a class="dropdown-item" href="{{ route('media.profile.edit') }}">{{ __('My Profile') }}</a>
                          @endif
                        </div>
                      </div>
                    </td>
