...
@section('pageTitle','Copa Tech - User Management')

@section('content')
  @component('dashboard.layouts.headers.auth')
    @component('dashboard.layouts.headers.breadcrumbs')
      <li class="breadcrumb-item"><a href="{{ route('communication.user.index') }}">{{ __('User Management') }}</a></li>
          ...
          <div class="card-header">
            <div class="row align-items-center">
              ...
              <div class="col-4 text-right">
                <a href="{{ route('communication.user.create') }}" class="btn btn-sm btn-primary">{{ __('Add user') }}</a>
              </div>
            </div>
          </div>
                  ...
                  <tr>
                    <td>
                      <span class="avatar avatar-sm rounded-circle">
                        <img src="{{ config('app.url') . $user->profilePicture() }}" alt="" style="max-width: 100px; border-radiu: 25px">
                      </span>
                    </td>
                    <td>{{ $user->name }}</td>
                    <td><a href="mailto:{{ $user->email }}">{{ $user->email }}</a></td>
                    <td>{{ $user->role->name }}</td>
                    <td>{{ $user->created_at->format('l, M d, Y') }}</td>
                    <td class="text-right">
                      <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                          @if ($user->id != auth()->id())
                            <a class="dropdown-item" href="{{ route('communication.user.edit', $user) }}">{{ __('Edit') }}</a>
                            <form action="{{ route('communication.user.destroy', $user) }}" method="post">
                              @csrf
                              @method('delete')
                          ...
                          @else
                            <a class="dropdown-item" href="{{ route('communication.profile.edit') }}">{{ __('My Profile') }}</a>
                          @endif
                        </div>
                      </div>
                    </td>
