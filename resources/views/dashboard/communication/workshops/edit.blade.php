...
@section('pageTitle','Copa Tech - Edit Workshop')

@section('content')
  @component('dashboard.layouts.headers.auth')
    @component('dashboard.layouts.headers.breadcrumbs')
      <li class="breadcrumb-item"><a href="{{ route('communication.workshop.index') }}">{{ __('Workshop Management') }}</a></li>
      <li class="breadcrumb-item active" aria-current="page">{{ __('Edit Workshop') }}</li>
    @endcomponent
  @endcomponent
          ...
          <div class="card-header">
            <div class="row align-items-center">
              ...
              <div class="col-4 text-right">
                <a href="{{ route('communication.workshop.index') }}" class="btn btn-sm btn-primary">{{ __('Back to list') }}</a>
              </div>
            </div>
          </div>
          <div class="card-body">
            <form method="post" class="item-form" action="{{ route('communication.workshop.update', $workshop) }}" autocomplete="off" enctype="multipart/form-data">
              @csrf
              @method('put')
              ...
              <div class="pl-lg-4">
                <div class="form-group{{ $errors->has('title') ? ' has-danger' : '' }}">
                  ...
                  <input type="text" name="title" id="input-title" class="form-control{{ $errors->has('title') ? ' is-invalid' : '' }}" placeholder="{{ __('Title') }}" value="{{ old('title', $workshop->title) }}"  autofocus>
                ...
                <div class="form-group{{ $errors->has('venue_id') ? ' has-danger' : '' }}">
                  ...
                  <select name="venue_id" id="input-role" class="form-control{{ $errors->has('venue_id') ? ' is-invalid' : '' }}" >
                    ...
                    @foreach ($venues as $venue)
                      <option value="{{ $venue->id }}" {{ $venue->id == old('venue_id', $workshop->venue_id) ? 'selected' : '' }}>{{ $venue->name }}</option>
                    @endforeach
                  </select>
                ...
                <div class="form-group{{ $errors->has('excerpt') ? ' has-danger' : '' }}">
                  ...
                  <textarea name="excerpt" id="input-excerpt" cols="30" rows="2" class="form-control{{ $errors->has('excerpt') ? ' is-invalid' : '' }}" placeholder="{{ __('Excerpt') }}" value="{{ old('excerpt') }}">{{ old('excerpt', $workshop->excerpt) }}</textarea>

                  @include('alerts.feedback', ['field' => 'excerpt'])
                </div>
                <div class="form-group{{ $errors->has('description') ? ' has-danger' : '' }}">
                  <label class="form-control-label" for="compose-textarea">{{ __('Description') }}</label>
                  <textarea name="description" id="compose-textarea" class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}" style="height: 300px" {{ __('Description') }}>{{ old('description', $workshop->description) }}</textarea>
                ...
                <div class="form-group{{ $errors->has('tags') ? ' has-danger' : '' }}">
                  ...
                  <select name="tags[]" id="input-tags" class="form-control select2{{ $errors->has('tags') ? ' is-invalid' : '' }}" placeholder="{{ __('Tags') }}" data-toggle="select"  multiple>
                    @foreach ($tags as $tag)
                      <option value="{{ $tag->id }}" {{ in_array($tag->id, old('tags', $workshop->tags->pluck('id')->toArray()) ?? []) ? 'selected' : '' }}>{{ $tag->name }}</option>
                    @endforeach
                  </select>
                ...
                <div class="form-group{{ $errors->has('status') ? ' has-danger' : '' }}">
                  ...
                  <select name="status" id="input-status" class="form-control{{ $errors->has('status') ? ' is-invalid' : '' }}" >
                    ...
                    @foreach (config('aekiti.workshops.status') as $value => $status)
                      <option value="{{ $value }}" {{ $value == old('status', $workshop->status) ? 'selected' : '' }}>{{ $status }}({{ $value }})</option>
                    @endforeach
                  </select>
                ...
                <div class="form-group{{ $errors->has('mode') ? ' has-danger' : '' }}">
                  ...
                  <select name="mode" id="input-mode" class="form-control{{ $errors->has('mode') ? ' is-invalid' : '' }}" placeholder="{{ __('Venue') }}" >
                    ...
                    @foreach (config('aekiti.workshops.mode') as $value => $mode)
                      <option value="{{ $value }}" {{ $value == old('mode', $workshop->mode) ? 'selected' : '' }}>{{ $mode }}({{ $value }})</option>
                    @endforeach
                  </select>
                ...
                <div class="form-group{{ $errors->has('link') ? ' has-danger' : '' }}">
                  ...
                  <input type="url" name="link" id="input-link" class="form-control{{ $errors->has('link') ? ' is-invalid' : '' }}" placeholder="{{ __('Registration Link') }}" value="{{ old('link', $workshop->link) }}">
                ...
                <div class="form-group{{ $errors->has('youtube') ? ' has-danger' : '' }}">
                  ...
                  <input type="url" name="youtube" id="input-youtube" class="form-control{{ $errors->has('youtube') ? ' is-invalid' : '' }}" placeholder="{{ __('Youtube') }}" value="{{ old('youtube', $workshop->youtube) }}">
                ...
                <div class="form-group{{ $errors->has('slide') ? ' has-danger' : '' }}">
                  ...
                  <input type="url" name="slide" id="input-slide" class="form-control{{ $errors->has('slide') ? ' is-invalid' : '' }}" placeholder="{{ __('Slide') }}" value="{{ old('slide', $workshop->slide) }}">

                  @include('alerts.feedback', ['field' => 'slide'])
                </div>
                <div class="form-group{{ $errors->has('photo') ? ' has-danger' : '' }}">
                  <label class="form-control-label" for="input-photo">{{ __('Photo') }}</label>
                  <input type="url" name="photo" id="input-photo" class="form-control{{ $errors->has('photo') ? ' is-invalid' : '' }}" placeholder="{{ __('Photo') }}" value="{{ old('photo', $workshop->photo) }}">
                ...
                <div class="row">
                  <div class="col-md-6">
                    <div class="form-group">
                      ...
                      value="{{ old('start_date', $workshop->start_date ? \Carbon\Carbon::parse($workshop->start_date)->format('Y/m/d H:i:s') : '')}}">
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      ...
                      <input class="form-control date" name="end_date" id="end_date" type="datetime" value="{{ old('end_date', $workshop->end_date ? \Carbon\Carbon::parse($workshop->end_date)->format('Y/m/d H:i:s') : '')}}">
...
@push('css')
  ...
  {{-- <link rel="stylesheet" href="{{ asset('assets/dashboard/plugins/bootstrap/datetimepicker/css/datetimepicker.min.css') }}"> --}}
@endpush
...
@push('js')
  ...
  <script src="{{ asset('assets/dashboard/plugins/bootstrap/datetimepicker/js/moment.js') }}"></script>
  {{-- <script src="{{ asset('assets/dashboard/plugins/bootstrap/datetimepicker/js/transition.js') }}"></script>
  <script src="{{ asset('assets/dashboard/plugins/bootstrap/datetimepicker/js/collapse.js') }}"></script> --}}
  {{-- <script src="{{ asset('assets/dashboard/plugins/bootstrap/datetimepicker/js/datetimepicker.min.js') }}"></script> --}}
