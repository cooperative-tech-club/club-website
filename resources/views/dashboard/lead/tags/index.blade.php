...
@section('content')
  @component('dashboard.layouts.headers.auth')
    @component('dashboard.layouts.headers.breadcrumbs')
      <li class="breadcrumb-item"><a href="{{ route('lead.tag.index') }}">{{ __('Tag Management') }}</a></li>
          ...
          <div class="card-header">
            <div class="row align-items-center">
              ...
              <div class="col-4 text-right">
                <a href="{{ route('lead.tag.create') }}" class="btn btn-sm btn-primary">{{ __('Add tag') }}</a>
              </div>
            </div>
          </div>
                    ...
                    <td class="text-right">
                      <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                          <a class="dropdown-item" href="{{ route('lead.tag.edit', $tag) }}">{{ __('Edit') }}</a>
                          @if ($tag->stories->isEmpty())
                            <form action="{{ route('lead.tag.destroy', $tag) }}" method="post">
                              @csrf
                              @method('delete')
