...
@section('pageTitle','Copa Tech - Role Management')

@section('content')
  @component('dashboard.layouts.headers.auth')
    @component('dashboard.layouts.headers.breadcrumbs')
      <li class="breadcrumb-item"><a href="{{ route('communication.role.index') }}">{{ __('Role Management') }}</a></li>
          ...
          <div class="card-header">
            <div class="row align-items-center">
              ...
              <div class="col-4 text-right">
                <a href="{{ route('communication.role.create') }}" class="btn btn-sm btn-primary">{{ __('Add role') }}</a>
            </div>
            </div>
          </div>
              ...
              <tbody>
                @foreach ($roles as $role)
                  <tr>
                    ...
                    <td>{{ $role->created_at->format('l, M d, Y') }}</td>
                    <td class="text-right">
                      <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                          <a class="dropdown-item" href="{{ route('communication.role.edit', $role) }}">{{ __('Edit') }}</a>
                        </div>
                    </div>
                  </td>
                  </tr>
