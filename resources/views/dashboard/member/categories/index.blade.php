...
@section('pageTitle','Copa Tech - Category Management')

@section('content')
  @component('dashboard.layouts.headers.auth')
    @component('dashboard.layouts.headers.breadcrumbs')
      <li class="breadcrumb-item"><a href="{{ route('member.category.index') }}">{{ __('Category Management') }}</a></li>
          ...
          <div class="card-header">
            <div class="row align-items-center">
              ...
              <div class="col-4 text-right">
                <a href="{{ route('member.category.create') }}" class="btn btn-sm btn-primary">{{ __('Add category') }}</a>
              </div>
            </div>
          </div>
              ...
              <tbody>
                @foreach ($categories as $category)
                  <tr>
                    ...
                    <td>{{ $category->created_at->format('l, M d, Y') }}</td>
                    <td class="text-right">
                      <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                          <a class="dropdown-item" href="{{ route('member.category.edit', $category) }}">{{ __('Edit') }}</a>
                          <form action="{{ route('member.category.destroy', $category) }}" method="post">
                            @csrf
                            @method('delete')
