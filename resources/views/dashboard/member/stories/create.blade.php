...
@section('pageTitle','Copa Tech - Add Story')

@section('content')
  @component('dashboard.layouts.headers.auth')
    @component('dashboard.layouts.headers.breadcrumbs')
      <li class="breadcrumb-item"><a href="{{ route('member.story.index') }}">{{ __('Story Management') }}</a></li>
      <li class="breadcrumb-item active" aria-current="page">{{ __('Add Story') }}</li>
    @endcomponent
  @endcomponent
          ...
          <div class="card-header">
            <div class="row align-items-center">
              ...
              <div class="col-4 text-right">
                <a href="{{ route('member.story.index') }}" class="btn btn-sm btn-primary">{{ __('Back to list') }}</a>
              </div>
            </div>
          </div>
          <div class="card-body">
            <form method="post" class="item-form" action="{{ route('member.story.store') }}" autocomplete="off" enctype="multipart/form-data">
              @csrf
              ...
              <div class="pl-lg-4">
                ...
                <div class="form-group{{ $errors->has('slug') ? ' has-danger' : '' }}">
                  ...
                  <input type="text" name="slug" id="input-slug" class="form-control{{ $errors->has('slug') ? ' is-invalid' : '' }}" placeholder="{{ __('Slug') }}" value="{{ old('slug') }}">
                ...
                <div class="form-group{{ $errors->has('article') ? ' has-danger' : '' }}">
                  ...
                  <textarea name="article" id="compose-textarea" class="form-control{{ $errors->has('article') ? ' is-invalid' : '' }}">{{ old('article') }}</textarea>
                  @include('alerts.feedback', ['field' => 'article'])
                </div>
                <div class="form-group{{ $errors->has('photo') ? ' has-danger' : '' }}">
                  <label class="form-control-label" for="pick-image">{{ __('Story Picture') }}</label>
                  <div class="custom-file">
                    <input type="file" name="photo" class="custom-file-input{{ $errors->has('photo') ? ' is-invalid' : '' }}" id="pick-image" accept="image/*">
                    <label class="custom-file-label" for="pick-image">{{ __('Select Story Picture') }}</label>
                  </div>
                ...
                <div class="form-group{{ $errors->has('status') ? ' has-danger' : '' }}">
                  ...
                  @foreach (config('aekiti.stories') as $value => $status)
                    <div class="custom-control custom-radio mb-3">
                      <input name="status" class="custom-control-input" id="{{ $value }}" value="{{ $value }}" type="radio" {{ old('status') == $value ? ' checked' : '' }} onclick="showDate('{{ $value }}')">
                ...
                <div class="row" id="show_date">
                  <div class="col-md-6">
                    <div class="form-group">
                      ...
                      <input class="form-control datepicker" name="published_date" id="published_date" placeholder="Select published date" type="text" data-date-format="dd-mm-yyyy" value="{{ old('published_date', now()->format('d-m-Y')) }}">
                    </div>
                  </div>
                </div>
