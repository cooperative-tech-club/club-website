...
@section('pageTitle','Copa Tech - Tag Management')

@section('content')
  @component('dashboard.layouts.headers.auth')
    @component('dashboard.layouts.headers.breadcrumbs')
      <li class="breadcrumb-item"><a href="{{ route('member.tag.index') }}">{{ __('Tag Management') }}</a></li>
          ...
          <div class="card-header">
            <div class="row align-items-center">
              ...
              <div class="col-4 text-right">
                <a href="{{ route('member.tag.create') }}" class="btn btn-sm btn-primary">{{ __('Add tag') }}</a>
              </div>
            </div>
          </div>
              ...
              <tbody>
                @foreach ($tags as $tag)
                  <tr>
                    ...
                    <td>{{ $tag->created_at->format('l, M d, Y') }}</td>
                    <td class="text-right">
                      <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                          <a class="dropdown-item" href="{{ route('member.tag.edit', $tag) }}">{{ __('Edit') }}</a>
                          @if ($tag->stories->isEmpty())
                            <form action="{{ route('member.tag.destroy', $tag) }}" method="post">
                              @csrf
                              @method('delete')
